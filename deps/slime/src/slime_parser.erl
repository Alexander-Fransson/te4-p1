...
'doctype'(Input, Index) ->
  p(Input, Index, 'doctype', fun(I,D) -> (p_seq([p_optional(fun 'space'/2), p_string(<<"doctype">>), fun 'space'/2, p_label('name', p_one_or_more(p_seq([p_not(fun 'eol'/2), p_anything()]))), fun 'eol'/2, p_zero_or_more(p_seq([p_optional(fun 'space'/2), fun 'crlf'/2]))]))(I,D) end, fun(Node, Idx) ->transform('doctype', Node, Idx) end).
...
'shortcut'(Input, Index) ->
  p(Input, Index, 'shortcut', fun(I,D) -> (p_seq([p_label('type', p_choose([p_string(<<".">>), p_string(<<"#">>), p_string(<<"@">>), p_string(<<"$">>), p_string(<<"%">>), p_string(<<"^">>), p_string(<<"&">>), p_string(<<"+">>), p_string(<<"!">>)])), p_label('value', fun 'shortcut_value'/2)]))(I,D) end, fun(Node, Idx) ->transform('shortcut', Node, Idx) end).
...
-spec 'shortcut_value'(input(), index()) -> parse_result().
'shortcut_value'(Input, Index) ->
  p(Input, Index, 'shortcut_value', fun(I,D) -> (p_one_or_more(p_seq([p_optional(p_charclass(<<"[:\/]">>)), p_charclass(<<"[a-zA-Z0-9_-]">>)])))(I,D) end, fun(Node, Idx) ->transform('shortcut_value', Node, Idx) end).
